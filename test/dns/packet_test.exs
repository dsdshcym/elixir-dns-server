defmodule DNS.PacketTest do
  ...
  describe "binary |> parse |> to_binary" do
    test "simple question" do
      binary =
        <<0xB6, 0xDA, 0x00, 0x20, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x05, 0x7A,
          0x68, 0x69, 0x68, 0x75, 0x03, 0x63, 0x6F, 0x6D, 0x00, 0x00, 0x01, 0x00, 0x01>>

      assert binary == binary |> Packet.parse() |> Packet.to_binary()
    end
  end
end
